...
void reconnect() {//等待，直到连接上服务器
  while (!client.connected()) {//如果没有连接上
    if (client.connect("hsjvna")+random(999999999)) {//接入时的用户名，尽量取一个很不常用的用户名
...
void setup() {//初始化程序，只运行一遍
  Serial.begin(4800);//设置串口波特率（与烧写用波特率不是一个概念）
...
void loop() {//主循环
  ...
  if (now - lastMsg > 1000) {//每隔1秒发一次信号
    ...
    //client.publish("temperature", msg);//发送数据，其中temperature是发出去的outtopic
